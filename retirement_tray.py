...
class RetirementTrayApp:
    ...
    def create_menu(self):
        """创建托盘右键菜单"""
        return pystray.Menu(
            pystray.MenuItem("打开倒计时", lambda: self.root.after(0, self.on_show_countdown)),
            pystray.MenuItem("设置", lambda: self.root.after(0, self.on_settings)),
            pystray.MenuItem("更新当前存款", lambda: self.root.after(0, self.on_update_deposit)),
            pystray.MenuItem.SEPARATOR,
            pystray.MenuItem("退出", lambda: self.root.after(0, self.on_exit))
        )
